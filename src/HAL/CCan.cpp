...
:CANx_(CANx),
	...
	lastErrCode_(0x00),
	errCheckTimer_(this, &CCanRouter::errCheckCallback, 50, true)
{
	if(CANx_ == CAN1) 
	...
	setAutoTransmit(true);
}

void CCanRouter::setAutoTransmit(bool is_enable)
{
	if (is_enable)
	{
		CANx_->MCR &= ~(uint32_t)CAN_MCR_NART;			//automatic transmit on
	}
	else
	{
		CANx_->MCR |= CAN_MCR_NART;			//automatic transmit off
	}
}
...
void CCanRouter::InitCan()
{
	...
	else if(baudRate_ == 20000)
	{
		CAN_InitStructure.CAN_Prescaler = 175;
	}	
	else if(baudRate_ == 50000)
	{
		CAN_InitStructure.CAN_Prescaler = 70;
	}
	else if(baudRate_ == 100000)
	{
		CAN_InitStructure.CAN_Prescaler = 35;
	}
	else if(baudRate_ == 125000)
	{
...
void CCanRouter::InitCanGpio(int IOGroup)
{
	...
	else if(IOGroup == GROUP_A11)
	{
		...
		GPIOx = GPIOD;
		GPIO_PinSource_BASE = GPIO_PinSource0;
	...
	GPIO_PinAFConfig(GPIOx, GPIO_PinSource0, GPIO_AF_CANx);
	GPIO_PinAFConfig(GPIOx, GPIO_PinSource1, GPIO_AF_CANx);	
...
bool CCanRouter::attachMailbox(CCanRxMailbox_base* pMailbox)
{
	...
	{
		Console::Instance()->printf("Mailbox %d already attached\r\n", pMailbox->stdId());
		return true;
	}
