...
def gyro_goto_emergency_commandhandler():
    count = 10
    while(count):
        f4kernelcommand(0x0000101A,[0x06])
        print("send 7F arrary start：", count)
        time.sleep(0.5)
        count = count - 1
...
def odojumpdebug_commandhandler():
    f4kernelcommand(0x00001033,[0x02])
