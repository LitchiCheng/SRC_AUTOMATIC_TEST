...
{
	public:
		CAsyncTimer(T* pObj, void(T::*pFunc)(), int32_t period, bool is_sys = false):
			CTimer_base(period, period, is_sys), _pObj(pObj), _pFunc(pFunc)
		{}
		virtual void callback()
		{
			if(_pObj && _pFunc)
...
{
	public:
		CAsyncTimer( void(* pFunc)(), int32_t period, bool is_sys = false):
			CTimer_base(period, period, is_sys), _pFunc(pFunc)
		{}
		virtual void callback()
		{
			if(_pFunc)
