...
void CBaseTimer::clearAsyncTimer(void)
{
	CTimer_base* tempT = NULL;
	int time_num = asyncTimerTab().size();
	fixed_vector<CTimer_base*, MAX_ASYNC_TIME_NUM> tempVec;

	for (int i = 0; i < time_num; i++)
	{
		tempT = asyncTimerTab().at(i);
		if (false == tempT->isSys())
		{
			tempVec.push_back(tempT);
		}
	}

	time_num = tempVec.size();
	for (int i = 0; i < time_num; ++i)
	{
		tempT = tempVec.at(i);
		asyncTimerTab().erase(asyncTimerTab().find(tempT));
	}
}
