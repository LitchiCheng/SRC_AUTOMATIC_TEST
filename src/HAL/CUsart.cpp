...
/**
  * @brief  Clean all the flag of DMARxStream
	* @param  None
  * @retval Noe
  */
void CUsart::cleanAllFlag()
{
	uint32_t * RXDMA_IFCR;
	uint32_t streamx = (((uint32_t)DmaTxStream_&0xFF) - 0x10)/0x18;
	DMA_TypeDef * DMAx = ((DMA_TypeDef *)((uint32_t)DmaTxStream_&0xFFFFFC00));
	if(streamx < 4) RXDMA_IFCR = (uint32_t*)&(DMAx->LIFCR);
	else RXDMA_IFCR = (uint32_t*)&(DMAx->HIFCR);
	*RXDMA_IFCR = 0;
}
